...
public:
		...
		:Layer("Example"), m_Camera(-1.6f, 1.6f, -0.9f, 0.9f), m_CameraPosition(0.0f)
	{
		...
		std::shared_ptr<Cicala::VertexBuffer> vertexBuffer;
		vertexBuffer.reset(Cicala::VertexBuffer::Create(vertices, sizeof(vertices)));
		Cicala::BufferLayout layout = {
			{ Cicala::ShaderDataType::Float3, "a_Position" },
			{ Cicala::ShaderDataType::Float4, "a_Color" }
		};
		vertexBuffer->SetLayout(layout);
		m_VertexArray->AddVertexBuffer(vertexBuffer);

		uint32_t indices[3] = { 0, 1, 2 };
		std::shared_ptr<Cicala::IndexBuffer> indexBuffer;
		...
		std::shared_ptr<Cicala::VertexBuffer> squareVB;
		squareVB.reset(Cicala::VertexBuffer::Create(squareVertices, sizeof(squareVertices)));
		squareVB->SetLayout({
			{ Cicala::ShaderDataType::Float3, "a_Position" }
			});
		...
		std::shared_ptr<Cicala::IndexBuffer> squareIB;
		squareIB.reset(Cicala::IndexBuffer::Create(squareIndices, sizeof(squareIndices) / sizeof(uint32_t)));
	...
	void OnUpdate(Cicala::Timestep ts) override
	{
		...
		std::dynamic_pointer_cast<Cicala::OpenGLShader>(m_FlatColorShader)->Bind();
		std::dynamic_pointer_cast<Cicala::OpenGLShader>(m_FlatColorShader)->UploadUniformFloat3("u_Color", m_SquareColor);
...
private:
	std::shared_ptr<Cicala::Shader> m_Shader;
	std::shared_ptr<Cicala::VertexArray> m_VertexArray;

	std::shared_ptr<Cicala::Shader> m_FlatColorShader;
	std::shared_ptr<Cicala::VertexArray> m_SquareVA;
